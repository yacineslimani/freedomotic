...
extends Protocol {
	...
	protected void saveInOntologie(){
		app.createObjectInstance("Time" ,"Time","hasSensorValue", 10.0);
		...
		for(EnvironmentLogic flate: getApi().environments().findAll()){
			...
			for(Room room : flate.getRooms()){	
				...
				for (EnvObjectLogic object : getApi().things().findAll()) {
					...
					if(object.getClass().toString().equals("class com.freedomotic.things.impl.Thermometer")){
						if(r.getPojo().getUuid().equals(room.getPojo().getUuid())){
							Thermometer termo = (Thermometer) object;
							termo.executeSetTemperature(450, new Config());
							 value = Double.parseDouble(termo.getBehavior("temperature").getValueAsString());
						}
					}
				...
				app.createObjectInstance("Temperature" ,room.getPojo().getName()+"Temperature","hasSensorValue", value);
				app.createObjectRelInstance(room.getPojo().getName(), "hasEnvironment","Time");
			...
			if (object.getClass().toString().equals("class com.freedomotic.things.impl.Person")) {
				...
				if (object.getClass().toString().equals("class com.freedomotic.things.impl.Light")){
					...
					if (object.getClass().toString().equals("class com.freedomotic.things.impl.AirConditioner")){
						Room r = getObjLocation(object);
						//ajouter Aircondition
						app.createObjectInstance("Aircondition" ,object.getPojo().getName(), "hasId",object.getPojo().getUUID());
						app.createObjectRelInstance(object.getPojo().getName(), "hasLocation", r.getPojo().getName());

						String state=null;
						for (BehaviorLogic air: object.getBehaviors()) {
							if( air.getName().equals("powered")){
								state= air.getValueAsString();
					...
					}else if  (object.getClass().toString().equals("class com.freedomotic.things.impl.Gate")){
						Room r = getObjLocation(object);
						app.createObjectInstance("Window" ,object.getPojo().getName(), "hasId",object.getPojo().getUUID());
						app.createObjectRelInstance(object.getPojo().getName(), "hasLocation", r.getPojo().getName());
						String state="";
						
						for (BehaviorLogic gate: object.getBehaviors()) {
							if( gate.getName().equals("open value")){
								state= gate.getValueAsString();
	...
	public void ramdomUserLocation(){
		
		int x[] = {689,985,462,155,155,499,785,1290,913};
		int y[] = {475,401,602,499,691,909,1044,493,369};
		int px=0,py=0;
		...
		for (EnvObjectLogic object : getApi().things().findAll()) {
			if (object.getClass().toString().equals("class com.freedomotic.things.impl.Person")){
				...
				//p.getPojo().getCurrentRepresentation().setOffset((int)px,(int)py);
				p.setLocation(px, py);
				p.setChanged(true);
		...
		protected void onRun(){
			...
			app = new OntologieApp();
			saveInOntologie();
			action.switchOnLight(getApi().things().findAll(), app);
			action.switchOffLight(getApi().things().findAll(), app);
			action.switchOnAirCondition(getApi().things().findAll(), app);
			action.openWindow(getApi().things().findAll(), app);
